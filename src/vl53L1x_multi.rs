...
const MULTIPLEXER_DELAY: u32 = 500;
const MULTI_CHAN_NUM: u8 = 1;
...
impl Vl53l1Multi{
    ...
    pub fn init_devices(&mut self){
            self.i2c.write(self.tcaaddr, &[1 <<  MULTI_CHAN_NUM]);
            self.delay.delay_us(MULTIPLEXER_DELAY);

            vl53l1::software_reset(&mut self.devs[0].device, &mut self.i2c, &mut self.delay);

            vl53l1::data_init(&mut self.devs[0].device, &mut self.i2c);

            vl53l1::static_init(&mut self.devs[0].device);

            vl53l1::set_distance_mode(&mut self.devs[0].device, vl53l1::DistanceMode::Short);
            ...
            vl53l1::set_user_roi(&mut self.devs[0].device, roi);

            vl53l1::set_measurement_timing_budget_micro_seconds(&mut self.devs[0].device, 20_000);
            vl53l1::set_inter_measurement_period_milli_seconds(&mut self.devs[0].device, 20);

            vl53l1::start_measurement(&mut self.devs[0].device, &mut self.i2c);
    }

    pub fn read_all(&mut self) -> u16{
      ...
      for i in 0..4 {
        ...
        let mut mes: u16 = 0;
        vl53l1::wait_measurement_data_ready(&mut self.devs[0].device, &mut self.i2c, &mut self.delay);
        let rmd =
            match vl53l1::get_ranging_measurement_data(&mut self.devs[0].device, &mut self.i2c)
            {
                Ok(x) => {
                    match x.range_status{
                        RangeStatus::RANGE_VALID => {x.range_milli_meter}
        ...
        mes = rmd as u16;
        vl53l1::clear_interrupt_and_start_measurement(&mut self.devs[0].device, &mut self.i2c, &mut self.delay);
