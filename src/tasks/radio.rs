...
use crate::radio::types::{Node, NodeState};
use crate::units::{us, MicroSeconds, MilliSeconds, U32UnitsExt};
use crate::util::{TraceEvent, Tracer};
use cfg_if::cfg_if;
...
static mut TRACER: RttTracer = RttTracer {
    prev: 0,
    prev_gts: 0,
    sysclk: 160_000_000,
};
    ...
    match e {
        ...
        GTSProcessingFinished => {
            ...
            if gts_started.is_some() {
                ...
                cx.resources.event_state_data.gts_elapsed =
                    us(cycles2us!(cx.resources.clocks, elapsed.as_cycles()) as u32);
            }
        }
        GTSShouldHaveEnded => {
        ...
        GTSStartReceived(time_mark) => {
            ...
            let gt_slot = cx
                .resources
                .radio
                .lock(|radio| radio.master.find_slot(SlotType::GtsUplink));
            match gt_slot {
                Some(s) => {
            ...
            let aloha_slot = cx
                .resources
                .radio
                .lock(|radio| radio.master.find_slot(SlotType::Aloha));
            match aloha_slot {
                Some(s) => {
                    let shift: MicroSeconds = s.shift;
                    let shift = us2cycles!(cx.resources.clocks, shift.0);
            ...
            let dyn_slot = cx
                .resources
                .radio
                .lock(|radio| radio.master.find_slot(SlotType::DynUplink));
            match dyn_slot {
                Some(s) => {
                    let shift: MicroSeconds = s.shift;
            ...
            let ranging_slot = cx
                .resources
                .radio
                .lock(|radio| radio.master.find_slot(SlotType::Ranging));
            match ranging_slot {
                Some(s) => {
                    let shift: MicroSeconds = s.shift;
                    let shift = us2cycles!(cx.resources.clocks, s.shift.0);
        ...
        DynProcessingFinished => {
            ...
            if dyn_started.is_some() {
                ...
                cx.resources.event_state_data.dyn_elapsed =
                    us(cycles2us!(cx.resources.clocks, elapsed.as_cycles()) as u32);
            }
        }
        DynShouldHaveEnded => {
        ...
        RangingSlotAboutToStart(ranging_slot_duration, radio_config) => {
            radio_command!(
                cx,
                Command::RangingStart(ranging_slot_duration, radio_config)
            );
        }
        RangingSlotEnded => {
            radio_command!(cx, Command::ForceReady);
